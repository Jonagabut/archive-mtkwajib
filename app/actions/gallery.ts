...
export async function uploadMediaAction(
  ...
  if (!ALLOWED_MIME_TYPES.has(mimeType)) {
    return { error: `Format tidak didukung: ${mimeType}. Gunakan JPEG, PNG, WebP, GIF, atau MP4.` };
  }
  ...
  const row = inserted as { id: string; storage_url: string };
  return { data: { id: row.id, storage_url: row.storage_url } };
}
